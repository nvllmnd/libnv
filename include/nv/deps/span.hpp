...
#if span_CONFIG_CONTRACT_VIOLATION_THROWS && span_CONFIG_CONTRACT_VIOLATION_TERMINATES
...
#define span_USES_STD_SPAN \
  ((span_CONFIG_SELECT_SPAN == span_SPAN_STD) || ((span_CONFIG_SELECT_SPAN == span_SPAN_DEFAULT) && span_HAVE_STD_SPAN))
...
class span {
 public:
  ...
  template <typename It span_REQUIRES_T((std::is_convertible<decltype(*std::declval<It&>()), element_type&>::value))>
  span_constexpr_exp span(It first, size_type count) : data_(to_address(first)), size_(count) {
    span_EXPECTS((data_ == span_nullptr && count == 0) || (data_ != span_nullptr && detail::is_positive(count)));
  }
#else
...
#else
  span_constexpr explicit span(std::initializer_list<value_type> il) span_noexcept : data_(il.begin()),
                                                                                     size_(il.size()) {
  }
#endif
  ...
  template <class OtherElementType,
            extent_type OtherExtent
                span_REQUIRES_T(((Extent == dynamic_extent || OtherExtent == dynamic_extent || Extent == OtherExtent) &&
                                 std::is_convertible<OtherElementType (*)[], element_type (*)[]>::value))>
  span_constexpr_exp span(span<OtherElementType, OtherExtent> const& other) span_noexcept : data_(other.data()),
                                                                                            size_(other.size()) {
