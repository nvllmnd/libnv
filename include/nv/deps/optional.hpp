...
#define optional_CPP11_140_C290_G490                                                      \
  ((optional_CPP11_OR_GREATER_ &&                                                         \
    (optional_COMPILER_CLANG_VERSION >= 290 || optional_COMPILER_GNUC_VERSION >= 490)) || \
...
#define optional_CPP11_110_C350_G500                                                    \
  (optional_CPP11_110 && !(optional_BETWEEN(optional_COMPILER_CLANG_VERSION, 1, 350) || \
...
union storage_t {
  ...
  void emplace(std::initializer_list<U> il, Args&&... args) {
    ::new (const_cast<void*>(static_cast<const volatile void*>(value_ptr())))
        value_type(il, std::forward<Args>(args)...);
  }
...
class optional {
 optional_static_assert((!std::is_same<typename std::remove_cv<T>::type, nullopt_t>::value),
     ...
     optional_static_assert((!std::is_same<typename std::remove_cv<T>::type, in_place_t>::value),
         ...
         optional_static_assert((std::is_object<T>::value && std::is_destructible<T>::value &&
 ...
 public:
  ...
  template <typename U = T optional_REQUIRES_T(std11::is_move_constructible<U>::value ||
  ...
  optional_constexpr explicit optional(nonstd_lite_in_place_t(T), Args&&... args)
      : has_value_(true), contained(in_place, std::forward<Args>(args)...) {}

  // 7 (C++11) - in-place construct,  initializer-list
  template <typename U,
            typename... Args optional_REQUIRES_T(std::is_constructible<T, std::initializer_list<U>&, Args&&...>::value)>
  ...
  optional_constexpr explicit optional(U&& value)
      : has_value_(true), contained(nonstd_lite_in_place(T), std::forward<U>(value)) {}
  ...
  optional_REQUIRES_R(optional&, true
                      ...
                      ) operator=(optional const& other) noexcept(std11::is_nothrow_move_assignable<T>::value &&
  ...
  optional_REQUIRES_R(optional&,
                      std::is_constructible<T, U>::value&& std11::is_assignable<T&, U>::value &&
                          !std::is_same<typename std20::remove_cvref<U>::type, nonstd_lite_in_place_t(U)>::value &&
                          !std::is_same<typename std20::remove_cvref<U>::type, optional<T> >::value &&
                          !(std::is_scalar<T>::value &&
                            std::is_same<T, typename std::decay<U>::type>::value)) operator=(U&& value) {
    if (has_value()) {
  ...
  optional_REQUIRES_R(
      optional&,
      std::is_constructible<T, U const&>::value&& std11::is_assignable<T&, U const&>::value &&
          ...
          !std::is_constructible<T, optional<U> const&&>::value && !std::is_convertible<optional<U>&, T>::value &&
          !std::is_convertible<optional<U>&&, T>::value && !std::is_convertible<optional<U> const&, T>::value &&
          !std::is_convertible<optional<U> const&&, T>::value && !std11::is_assignable<T&, optional<U>&>::value &&
          !std11::is_assignable<T&, optional<U>&&>::value && !std11::is_assignable<T&, optional<U> const&>::value &&
  ...
  optional_REQUIRES_R(
      ...
      std::is_constructible<T, U>::value&& std11::is_assignable<T&, U>::value &&
          ...
          !std::is_convertible<optional<U> const&&, T>::value && !std11::is_assignable<T&, optional<U>&>::value &&
          !std11::is_assignable<T&, optional<U>&&>::value && !std11::is_assignable<T&, optional<U> const&>::value &&
          !std11::is_assignable<T&, optional<U> const&&>::value) operator=(optional<U>&& other) {
...
#if optional_CPP11_OR_GREATER
      noexcept(std11::is_nothrow_move_constructible<T>::value && std17::is_nothrow_swappable<T>::value)
#endif
  {
    using std::swap;
  ...
  /*optional_nodiscard*/ optional_constexpr bool has_value() const optional_noexcept { return has_value_; }
...
template <typename T>
optional_nodiscard optional_constexpr bool operator<(optional<T> const& /*unused*/,
...
template <typename T>
optional_nodiscard optional_constexpr bool operator<=(nullopt_t /*unused*/,
...
template <typename T
#if optional_CPP11_OR_GREATER
              optional_REQUIRES_T(std11::is_move_constructible<T>::value&& std17::is_swappable<T>::value)
#endif
          >
