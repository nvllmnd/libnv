...
#define type_REQUIRES_0(...) template <bool B = (__VA_ARGS__), typename std::enable_if<B, int>::type = 0>

#define type_REQUIRES_T(...) , typename std::enable_if<(__VA_ARGS__), int>::type = 0

#define type_REQUIRES_R(R, ...) typename std::enable_if<(__VA_ARGS__), R>::type

#define type_REQUIRES_A(...) , typename std::enable_if<(__VA_ARGS__), void*>::type = nullptr
...
#define type_DEFINE_TYPE_CD(type_name, type_type, underlying_type, default_value)         \
  typedef ::nonstd::type_type<underlying_type, struct type_name##_tag,                    \
                              ::nonstd::custom_default_t<underlying_type, default_value>> \
      type_name;
...
#define type_DEFINE_FUNCTION(type_type, type_function, function) \
  inline type_type type_function(type_type const& x) { return type_type(function(x.get())); }

/**
 * define a function for given type, constexpr.
 */
#define type_DEFINE_FUNCTION_CE(type_type, type_function, function) \
  inline type_constexpr14 type_type type_function(type_type const& x) { return type_type(function(x.get())); }
...
struct is_eq {
  friend type_constexpr14 bool operator==(T const& x, U const& y) { return x.get() == y.get(); }
};
template <typename T, typename U = T>
struct is_lt {
  friend type_constexpr14 bool operator<(T const& x, U const& y) { return x.get() < y.get(); }
...
template <typename T, typename U = T>
struct is_ne {
  friend type_constexpr14 bool operator!=(T const& x, U const& y) { return !(x == y); }
};
template <typename T, typename U = T>
struct is_lteq {
  friend type_constexpr14 bool operator>(T const& x, U const& y) { return y < x; }
};
template <typename T, typename U = T>
struct is_gt {
  friend type_constexpr14 bool operator<=(T const& x, U const& y) { return !(y < x); }
...
struct logical_not {
  friend type_constexpr14 R operator!(T const& x) { return R(!x.get()); }
};

template <typename R, typename T = R, typename U = R>
struct logical_and {
  friend type_constexpr14 R operator&&(T const& x, U const& y) { return R(x.get() && y.get()); }
};
template <typename R, typename T = R, typename U = R>
struct logical_or {
  friend type_constexpr14 R operator||(T const& x, U const& y) { return R(x.get() || y.get()); }
...
template <typename R, typename T = R, typename U = R>
struct plus {
  friend type_constexpr14 R operator+(T x, U const& y) { return x += y; }
};
template <typename R, typename T = R, typename U = R>
struct plus2 {
  friend type_constexpr14 R operator+(T const& x, U y) { return y += x; }
};
template <typename R, typename T = R, typename U = R>
struct minus {
  friend type_constexpr14 R operator-(T x, U const& y) { return x -= y; }
...
// template< typename R, typename T = R > struct bit_not{ friend type_constexpr14 R operator~( T const & x ) { return
// ~x; }; };
...
template <typename R, typename T = R>
struct bit_shl {
  friend type_constexpr14 R operator<<(T x, int const n) { return x <<= n; }
};
template <typename R, typename T = R>
struct bit_shr {
  friend type_constexpr14 R operator>>(T x, int const n) { return x >>= n; }
...
struct data {
  ...
  type_constexpr data() : value() {}

#if type_CPP11_OR_GREATER
  type_constexpr explicit data(T v) : value(std::move(v)) {}
#else
  type_constexpr explicit data(T const& v) : value(v) {}
#endif
...
struct default_value {
  static type_constexpr T get() { return T(); }
};

template <typename T, T Val>
struct default_value<T, custom_default_t<T, Val>> {
  static type_constexpr T get() { return Val; }
};
...
#if type_CPP11_OR_GREATER
  type_REQUIRES_0((!std::is_same<D, no_default_t>::value))
#endif
      type_constexpr type()
...
#if type_CPP11_OR_GREATER
  type_constexpr explicit type(T v) : data<T, D>(std::move(v)) {}
#else
  type_constexpr explicit type(T const& v) : data<T, D>(v) {}
#endif
};
...
struct boolean : type<bool, Tag, D>, is_eq<boolean<Tag, D>>, is_ne<boolean<Tag, D>>, logical_not<boolean<Tag, D>> {
 private:
 ...
 public:
  ...
  type_constexpr explicit boolean(bool value) : type<bool, Tag, D>(value) {}

  // safe conversion to 'boolean';

#if type_HAVE_EXPLICIT_CONVERSION
  type_constexpr explicit operator bool() const { return this->get(); }
#else
...
#if type_CPP11_OR_GREATER
  type_constexpr explicit logical(T v) : type<T, Tag, D>(std::move(v)) {}
#else
  type_constexpr explicit logical(T const& v) : type<T, Tag, D>(v) {}
#endif
};
 ...
 */
template <typename T, typename Tag, typename D = T>
struct equality : type<T, Tag, D>, is_eq<equality<T, Tag, D>>, is_ne<equality<T, Tag, D>> {
#if type_CPP11_OR_GREATER
  type_REQUIRES_0(!std::is_same<D, no_default_t>::value)
#endif
...
#if type_CPP11_OR_GREATER
  type_constexpr explicit equality(T v) : type<T, Tag, D>(std::move(v)) {}
#else
  type_constexpr explicit equality(T const& v) : type<T, Tag, D>(v) {}
#endif
};
...
struct bits : equality<T, Tag, D>
...
#if type_CPP11_OR_GREATER
  type_constexpr explicit bits(T v) : equality<T, Tag, D>(std::move(v)) {}
#else
  type_constexpr explicit bits(T const& v) : equality<T, Tag, D>(v) {}
#endif

  type_constexpr14 bits operator~() { return bits(static_cast<T>(~this->get())); }

  type_constexpr14 bits& operator^=(bits const& other) {
    this->get() = this->get() ^ other.get();
    return *this;
  }
  type_constexpr14 bits& operator&=(bits const& other) {
    this->get() = this->get() & other.get();
  ...
  type_constexpr14 bits& operator<<=(int const n) {
    this->get() <<= n;
    return *this;
  }
  type_constexpr14 bits& operator>>=(int const n) {
 ...
 */
template <typename T, typename Tag, typename D = T>
struct ordered : equality<T, Tag, D>,
...
#if type_CPP11_OR_GREATER
  type_constexpr explicit ordered(T v) : equality<T, Tag, D>(std::move(v)) {}
#else
  type_constexpr explicit ordered(T const& v) : equality<T, Tag, D>(v) {}
#endif
};
 ...
 */
template <typename T, typename Tag, typename D = T>
struct numeric : ordered<T, Tag, D>,
...
#if type_CPP11_OR_GREATER
  type_constexpr explicit numeric(T v) : ordered<T, Tag, D>(std::move(v)) {}
#else
  type_constexpr explicit numeric(T const& v) : ordered<T, Tag, D>(v) {}
#endif
  ...
  type_constexpr14 numeric operator++(int) {
    numeric tmp(*this);
    ++*this;
    return tmp;
  }
  ...
  type_constexpr14 numeric& operator+=(numeric const& other) {
    this->get() += other.get();
    return *this;
  }
  type_constexpr14 numeric& operator-=(numeric const& other) {
 ...
 */
template <typename T, typename Tag, typename D = T>
struct quantity : ordered<T, Tag, D>,
...
#if type_CPP11_OR_GREATER
  type_constexpr explicit quantity(T v) : ordered<T, Tag, D>(std::move(v)) {}
#else
  type_constexpr explicit quantity(T const& v) : ordered<T, Tag, D>(v) {}
#endif

  type_constexpr14 quantity operator+() const { return *this; }
  type_constexpr14 quantity operator-() const { return quantity(-this->get()); }

  type_constexpr14 quantity& operator+=(quantity const& other) {
    this->get() += other.get();
    return *this;
  }
  type_constexpr14 quantity& operator-=(quantity const& other) {
 ...
 */
template <typename T, typename Tag, typename D = T>
struct offset : ordered<T, Tag, D>, plus<offset<T, Tag, D>>, minus<offset<T, Tag, D>> {
#if type_CPP11_OR_GREATER
  type_REQUIRES_0(!std::is_same<D, no_default_t>::value)
#endif
...
#if type_CPP11_OR_GREATER
  type_constexpr explicit offset(T v) : ordered<T, Tag, D>(std::move(v)) {}
#else
  type_constexpr explicit offset(T const& v) : ordered<T, Tag, D>(v) {}
#endif

  type_constexpr14 offset& operator+=(offset const& y) {
    this->get() += y.get();
    return *this;
  }
  type_constexpr14 offset& operator-=(offset const& y) {
...
struct address : ordered<T, Tag, D>,
...
#if type_CPP11_OR_GREATER
  type_constexpr explicit address(T v) : ordered<T, Tag, D>(std::move(v)) {}
#else
  type_constexpr explicit address(T const& v) : ordered<T, Tag, D>(v) {}
#endif

  type_constexpr14 address& operator+=(offset_type const& y) {
    this->get() += y.get();
    return *this;
  }
  type_constexpr14 address& operator-=(offset_type const& y) {
  ...
  friend type_constexpr14 offset_type operator-(address const& x, address const& y) {
    return offset_type(x.get() - y.get());
  }
};
...
template <typename T, typename Tag, typename D>
inline type_constexpr14 typename type<T, Tag, D>::underlying_type&& to_value(type<T, Tag, D>&& v) {
...
template <typename T, typename Tag, typename D>
inline type_constexpr14 typename type<T, Tag, D>::underlying_type const& to_value(type<T, Tag, D> const& v) {
  return v.get();
}
