...
constexpr auto make_unexpected(std::in_place_t inplace, Args&&... args)
...
// #if 0 && nsel_HAVE_ADDRESSOF
  ...
  class storage_t_noncopy_nonmove_impl {
   ...
   public:
    ...
    void construct_value() { new (std11::addressof(m_value)) value_type(); }
    ...
    value_type const* value_ptr() const { return std11::addressof(m_value); }

    value_type* value_ptr() { return std11::addressof(m_value); }
  ...
  class storage_t_impl {
   ...
   public:
    ...
    void construct_value(value_type const& e) { new (std11::addressof(m_value)) value_type(e); }

    void construct_value(value_type&& e) { new (std11::addressof(m_value)) value_type(std::move(e)); }
    ...
    void construct_error(error_type const& e) { new (std11::addressof(m_error)) error_type(e); }

    void construct_error(error_type&& e) { new (std11::addressof(m_error)) error_type(std::move(e)); }
  ...
  template <
      ...
      typename... Args nsel_REQUIRES_T(std::is_function<FnT>::value &&
  ...
  nsel_constexpr auto invoke_member_function_impl(FnT ClassT::* memfnptr, ObjectT&& obj, Args&&... args) noexcept(
  ...
  template <
      ...
      typename ObjectT nsel_REQUIRES_T(std::is_same<ClassT, typename std20::remove_cvref<ObjectT>::type>::value ||
                                       std::is_base_of<ClassT, typename std20::remove_cvref<ObjectT>::type>::value)>
  nsel_constexpr auto invoke_member_object_impl(MemberT ClassT::* memobjptr,
                                                ObjectT&& obj) noexcept(noexcept(std::forward<ObjectT>(obj).*memobjptr))
  ...
  template <typename MemberT, typename ClassT,
            typename ObjectT nsel_REQUIRES_T(is_reference_wrapper<typename std20::remove_cvref<ObjectT>::type>::value)>
  nsel_constexpr auto invoke_member_object_impl(MemberT ClassT::* memobjptr,
                                                ObjectT&& obj) noexcept(noexcept(obj.get().*memobjptr))
  ...
  nsel_constexpr auto invoke_member_object_impl(MemberT ClassT::* memobjptr, ObjectT&& obj) noexcept(
      noexcept((*std::forward<ObjectT>(obj)).*memobjptr)) -> decltype((*std::forward<ObjectT>(obj)).*memobjptr) {
    return (*std::forward<ObjectT>(obj)).*memobjptr;
  }

  template <typename F, typename... Args nsel_REQUIRES_T(
  ...
  nsel_constexpr auto invoke(F&& f,
                             Args&&... args) noexcept(noexcept(invoke_member_object_impl(std::forward<F>(f),
  ...
  using invoke_result_nocvref_t = typename std20::remove_cvref<decltype(::nonstd::expected_lite::detail::invoke(
      std::declval<F>(), std::declval<Args>()...))>::type;

#if nsel_P2505R >= 5
  template <typename F, typename... Args>
  using transform_invoke_result_t = typename std::remove_cv<decltype(::nonstd::expected_lite::detail::invoke(
      std::declval<F>(), std::declval<Args>()...))>::type;
#else
  template <typename F, typename... Args>
  ...
  struct valid_expected_value_type
      : std::integral_constant<bool, std::is_destructible<T>::value && !std::is_reference<T>::value &&
...
#endif  // nsel_P0323R
  {
   public:
    ...
    constexpr /*non-explicit*/ unexpected_type(unexpected_type<E2> const& error) : m_error(error.error()) {}
    ...
    nsel_constexpr14 E& error() & noexcept { return m_error; }

    constexpr E const& error() const& noexcept { return m_error; }

#if !nsel_COMPILER_GNUC_VERSION || nsel_COMPILER_GNUC_VERSION >= 490

    nsel_constexpr14 E&& error() && noexcept { return std::move(m_error); }

    constexpr E const&& error() const&& noexcept { return std::move(m_error); }
  ...
  // "the type of the unexpected value. The type must not be an array type, a non-object type, a specialization of
  // std::unexpected, or a cv-qualified type."
  template <typename T>
  struct valid_unexpected_type
      : std::integral_constant<bool, std::is_same<T, typename std20::remove_cvref<T>::type>::value &&
...
#endif  // nsel_P0323R
  {
   ...
   public:
    ...
    nsel_REQUIRES_0(std::is_default_constructible<T>::value) nsel_constexpr14 expected() : contained(true) {
    ...
    nsel_constexpr14 explicit expected(U&& value) noexcept(std::is_nothrow_move_constructible<U>::value &&
    ...
    nsel_constexpr14 /*non-explicit*/ expected(U&& value) noexcept(std::is_nothrow_move_constructible<U>::value &&
                                                                   std::is_nothrow_move_constructible<E>::value)
        : contained(true) {
    ...
    nsel_constexpr14 /*non-explicit*/ expected(nonstd::unexpected_type<G> const& error) : contained(false) {
    ...
    nsel_constexpr14 /*non-explicit*/ expected(nonstd::unexpected_type<G>&& error) : contained(false) {
    ...
    nsel_constexpr14 explicit expected(nonstd_lite_in_place_t(T), Args&&... args) : contained(true) {
    ...
    template <typename U,
              typename... Args nsel_REQUIRES_T(std::is_constructible<E, std::initializer_list<U>, Args&&...>::value)>
    nsel_constexpr14 explicit expected(unexpect_t, std::initializer_list<U> il, Args&&... args) : contained(false) {
    ...
    template <
        typename U nsel_REQUIRES_T(!std::is_same<expected<T, E>, typename std20::remove_cvref<U>::type>::value &&
...
#if nsel_P2505R >= 4
    template <typename G = E nsel_REQUIRES_T(std::is_copy_constructible<E>::value&& std::is_convertible<G, E>::value)>
    nsel_constexpr error_type error_or(G&& e) const& {
      return has_value() ? static_cast<E>(std::forward<G>(e)) : contained.error();
    }

    template <typename G = E nsel_REQUIRES_T(std::is_move_constructible<E>::value&& std::is_convertible<G, E>::value)>
    ...
    nsel_constexpr14 detail::invoke_result_nocvref_t<F, value_type&> and_then(F&& f) & {
      return has_value() ? detail::invoke_result_nocvref_t<F, value_type&>(detail::invoke(std::forward<F>(f), value()))
    ...
    template <typename F nsel_REQUIRES_T(
        detail::is_expected<detail::invoke_result_nocvref_t<F, const value_type&> >::value&&
            std::is_same<typename detail::invoke_result_nocvref_t<F, const value_type&>::error_type,
    ...
    template <typename F nsel_REQUIRES_T(
        detail::is_expected<detail::invoke_result_nocvref_t<F, const error_type&> >::value&&
            std::is_same<typename detail::invoke_result_nocvref_t<F, const error_type&>::value_type,
    ...
    template <typename F nsel_REQUIRES_T(
        detail::is_expected<detail::invoke_result_nocvref_t<F, const error_type&&> >::value&&
            std::is_same<typename detail::invoke_result_nocvref_t<F, const error_type&&>::value_type,
                         value_type>::value&& std::is_constructible<value_type, const value_type&&>::value)>
    nsel_constexpr detail::invoke_result_nocvref_t<F, const error_type&&> or_else(F&& f) const&& {
      return has_value() ? detail::invoke_result_nocvref_t<F, const error_type&&>(std::move(value()))
    ...
    nsel_constexpr expected<detail::transform_invoke_result_t<F, const value_type&>, error_type> transform(
        F&& f) const& {
    ...
    nsel_constexpr expected<value_type, detail::transform_invoke_result_t<F, const error_type&> > transform_error(
        F&& f) const& {
      return has_value()
    ...
    template <typename F nsel_REQUIRES_T(
        detail::valid_unexpected_type<detail::transform_invoke_result_t<F, error_type&&> >::value&&
            std::is_constructible<value_type, value_type&&>::value)>
    nsel_constexpr14 expected<value_type, detail::transform_invoke_result_t<F, error_type&&> > transform_error(
        F&& f) && {
...
#if nsel_P2505R >= 3
    // Monadic operations (P2505)
    template <typename F nsel_REQUIRES_T(detail::is_expected<detail::invoke_result_nocvref_t<F> >::value&& std::is_same<
  ...
  template <typename T, typename E nsel_REQUIRES_T((std::is_void<T>::value || std::is_move_constructible<T>::value) &&
