...
/// Strings are all null-terminated and length prefixed
...
PURE_FUNC static inline bool rune_strongeq(Rune left, Rune right) {
  ...
  const sslice l = runetab_lookup_slice(left.parent, left);
  const sslice r = runetab_lookup_slice(right.parent, right);
